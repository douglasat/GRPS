...
def plot_BarGraph_General():
    ...
    average_grps = [scenario_results[f'ppv_{a}'].mean() * 100 for a in [30, 50, 60, 100]]
    margin_error_grps = [100*1.96*scenario_results[f'ppv_{a}'].std() / np.sqrt(len(scenario_results)) for a in [30, 50, 60, 100]] 

    # GRPS+DTW
    scenario_results = pd.read_csv('./results/GRPS+DTW_15.csv')
    average_grps_dtw = [scenario_results[f'ppv_{a}'].mean() * 100 for a in [30, 50, 60, 100]]
    margin_error_grps_dtw = [100*1.96*scenario_results[f'ppv_{a}'].std() / np.sqrt(len(scenario_results)) for a in [30, 50, 60, 100]] 
    ...
    ax.set_title('(b) Discrete Domains', fontsize=24)
    ax.set_xlabel('Percentage of Observation', fontsize=24)
    ax.set_ylabel('PPV(%)', fontsize=24)
    ax.set_xticks(bar_positions)
    ax.set_xticklabels(observations, fontsize=24)
    ax.tick_params(axis='y', labelsize=24)
    ax.legend(fontsize=24, loc='upper center', ncol=3)
    ax.set_ylim(0, 85)
