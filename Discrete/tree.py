...
class Tree():
    ...
    def merge(self, node: Node = None, reduce: str = "average") -> None:
        ...
        for i, child in enumerate(node.children):
            for sibling in node.children[i + 1:]:
                node_distance = distance.euclidean(np.array(child.identifier),np.array(sibling.identifier)) / np.sqrt(len(child.identifier))
    ...
    def prune(self, node = None) -> None:
        ...
        for i, child in enumerate(node.children):
            node_distance = distance.euclidean(np.array(node.identifier),np.array(child.identifier)) / np.sqrt(len(child.identifier))
