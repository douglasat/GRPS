...
import vector_inference_plan_recognizer
import vector_path_signature_plan_recognizer
import mirroring_plan_recognizer
import mirroring_with_landmarks_plan_recognizer
import landmarks_plan_recognizer
import vector_inference_landmark_plan_recognizer
...
class PlanRecognizerFactory(object):
    ...
    def get_recognizers(self):
        ...
        mirroring_recognizers = dict([(cls.name, cls) for name, cls in mirroring_plan_recognizer.__dict__.items() if
                                      isinstance(cls, object) and hasattr(cls, 'name')])
        vector_inference_recognizers = dict([(cls.name, cls) for name, cls in vector_inference_plan_recognizer.__dict__.items() if
                                      isinstance(cls, object) and hasattr(cls, 'name')])
        vector_path_signature_recognizer = dict([(cls.name, cls) for name, cls in vector_path_signature_plan_recognizer.__dict__.items() if
                                      isinstance(cls, object) and hasattr(cls, 'name')])
        vector_inference_landmark_recognizer = dict(
            [(cls.name, cls) for name, cls in vector_inference_landmark_plan_recognizer.__dict__.items() if
             isinstance(cls, object) and hasattr(cls, 'name')])
        mirroring_landmarks_recognizers = dict([(cls.name, cls) for name, cls in mirroring_with_landmarks_plan_recognizer.__dict__.items() if
                                       isinstance(cls, object) and hasattr(cls, 'name')])
        landmarks_recognizer = dict(
            [(cls.name, cls) for name, cls in landmarks_plan_recognizer.__dict__.items() if
             isinstance(cls, object) and hasattr(cls, 'name')])

        h_recognizers.update(mirroring_recognizers)
        h_recognizers.update(vector_inference_recognizers)
        h_recognizers.update(mirroring_landmarks_recognizers)
        h_recognizers.update(landmarks_recognizer)
        h_recognizers.update(vector_inference_landmark_recognizer)
        h_recognizers.update(vector_path_signature_recognizer)
