...
class Hypothesis:
    ...
    def evaluate(self, observations):
        ...
        fd_cmd = FDCommand('domain.pddl', hyp_problem, self.opts)
        # fd_cmd.execute()
        # self.load_plan('ocsingleshot_heuristic_result.dat')
    ...
    def load_plan(self, plan_name):
        ...
        for line in instream:
            ...
            if not line[0] == '(':
                continue
            stuff1, _, stuff2 = custom_partition(line, ')')
            # op, _, _ = custom_partition(stuff, '[')
            self.plan.append(stuff1.strip().lower())
            self.planCost.append(stuff2.strip().lower())
