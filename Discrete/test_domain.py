...
def do_experiments(base_path, domain_name, observability, recognizer, opt):
    ...
    file_latex.write("#P\tO%\t|O|\t|G|\t|S|\tPPV\tSTD\tAcc\tSTD\tSpread\tPER\tOnline\tOffline\tPC\tOPN\n")

    for obs in observability:
        current_problem = 0
        exp_dir = domain_name + '/' + '100' + '/'
        ...
        for exp_file in files:
            ...
            if method == 'GRPS' or method == 'GRPS+DTW':
                recognizer.run_recognizer(obs, exp_file, merge, prune, method, topk)
            elif method == 'vector_inference':
                recognizer.run_recognizer(obs, topk)
            else:
                recognizer.run_recognizer(obs)
            # Log
