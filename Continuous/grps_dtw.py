...
def getEstimationPath(init, goals, data_memorize):  # compute an estimation from init to each goal in the set
    ...
    return sig_branch_by_goal, sig_level_by_goal, data_problem, offline_time
...
def vector_inference_multi(initial, goal):
    ...
    nodes_traj, _, data_problem, offline_time = getEstimationPath(state_init, scenario.goalPoints, data_memorize)
        ...
        for goal_Hypothese in goal_Hypothesis:
            ...
            for traj in nodes_traj[str(goal_Hypothese)]:
                if len(traj) > 1:
                    ...
                    distance_dtw = np.linalg.norm(sig_path_observations[-1] - traj[path[obs_index, 1]])
...
if __name__ == "__main__":
    ...
    # output files
    output = output.OutputData(scenario.name, scenario.num_obser, len(groupPoints[0]) - 1, 'grps_dtw')
    ...
    for points in groupPoints:
        ...
        # obs = vector_inference_multi(5, 3)[0]
        # print(obs[0:6])
